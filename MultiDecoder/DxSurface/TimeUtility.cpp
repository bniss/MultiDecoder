...
ETB *g_petb = new ETB;
using namespace std;
auto_ptr<ETB> g_ETBPtr(g_petb);

void InitHighPerformanceClock(ETB *petb)
{
#if _DEBUG
	OutputDebugStringA("***********************************************************************************\n");
	OutputDebugStringA("*	Warning:InitHighPerformanceClock() is deprecated.It's not recommended to call.	*\n");
	OutputDebugStringA("***********************************************************************************\n");
#endif
	if (petb == NULL)
	{
	...
	ZeroMemory(petb, sizeof(ETB));
	SYSTEMTIME systime;
	GetSystemTime(&systime);
	SystemTime2UTC(&systime, (UINT64*)&petb->nBaseClock);
	petb->dfMilliseconds = (double)(systime.wMilliseconds / 1000);
	...
	petb->dfFreq = LarInt.QuadPart;
	QueryPerformanceCounter(&LarInt);
	petb->dfCounter = LarInt.QuadPart;

}

//取得精确时间,单位秒
double  GetExactTime(ETB *petb)
{
	if (petb == NULL)
		petb = g_petb;
	assert(petb->dfFreq != 0);
	assert(petb->dfCounter != 0);
	LONGLONG dfCounter1;
	LARGE_INTEGER LarInt;
	QueryPerformanceCounter(&LarInt);
	dfCounter1 = LarInt.QuadPart;
	double dfReturn = (double)(petb->nBaseClock + petb->dfMilliseconds + (double)(dfCounter1 - petb->dfCounter) / petb->dfFreq);
	return dfReturn;
}
